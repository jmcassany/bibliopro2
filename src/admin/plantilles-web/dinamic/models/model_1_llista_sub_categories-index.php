...
//if ($CATEGORY2=='0')   { $CARDS_LISTFILTER .=" AND (CATEGORY2='0')"; }


if ($CATEGORY2 != 0 && cat_te_Descendents($CATEGORY2)) {
    $array_ids_fills_categoria = cat_get_IdDescendents($ID_CARPETA, "category_id=$CATEGORY2");
    $sql_categories_filles = implode(" OR CATEGORY2=", $array_ids_fills_categoria);
    $CARDS_LISTFILTER.= " AND ( CATEGORY2 = $CATEGORY2 OR CATEGORY2=$sql_categories_filles )";
}
...
if ($CATEGORY2 != 0 && cat_te_Descendents($CATEGORY2)) {
    $pagemax = $dbCards->countCardPages($CATEGORY1, 0);
} else {
...
if ($data['LLISTA_CATEGORIES'] != '') {
    $data['LLISTA_CATEGORIES'] = '<h2>Categories</h2><ul>' . $data['LLISTA_CATEGORIES'] . '<li><a href="'.$pathEditora.'index.php">Totes les notícies</a></li></ul>';
}

// selector de categories
$data['SELECT_CATEGORIES'] = '<select name="CATEGORY2" onchange="submit();">
								' . getSelectOptionsCategories($ID_CARPETA, "selected_category=" . $CATEGORY2 . "&default_option_text=Totes les categories") . '
							   </select>';
