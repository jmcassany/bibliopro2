...
for ($i = 0;$i < $total;$i++) {
    ...
    if ($data['DESCRIPCIO'] != '') {
        if ($urlFriendly) {
            $data['MES'] = '<p class="mes"><a href="'.$pathEditora.''.$idnow.'/'.$data['URL_TITOL'].'?KeepThis=true&amp;TB_iframe=true&amp;height=400&amp;width=600" class="thickbox">Més <span>informació sobre "' . $data['TITOL'] . '"</span></a></p>';
            $data['TITOL'] = '<a href="'.$pathEditora.''.$idnow.'/'.$data['URL_TITOL'].'?KeepThis=true&amp;TB_iframe=true&amp;height=400&amp;width=600"  class="thickbox">' . $data['TITOL'] . '</a>';
        }
        else {
            $data['MES'] = '<p class="mes"><a href="'.$pathEditora.'view.php?ID=' . $idnow . '&amp;KeepThis=true&amp;TB_iframe=true&amp;height=400&amp;width=600" class="thickbox">Més <span>informació sobre "' . $data['TITOL'] . '"</span></a></p>';
            $data['TITOL'] = '<a href="'.$pathEditora.'view.php?ID=' . $idnow . '&amp;KeepThis=true&amp;TB_iframe=true&amp;height=400&amp;width=600"  class="thickbox">' . $data['TITOL'] . '</a>';
        }
    }
    ...
    if ($data['CATEGORY2'] != 0) {
		if ($urlFriendly) {
			...
			$data['CATEGORIA'] = '<p class="categoria"><a href="'.$pathEditora.'cat/' . $data['CATEGORY2'] . '" title="Veure totes les activitats de la categoria ' . getNomCategoria($data['CATEGORY2']) . '/'.$infocat['URL_NOM'].'">' . getNomCategoria($data['CATEGORY2']) . '</a></p>';
		}
		else {
