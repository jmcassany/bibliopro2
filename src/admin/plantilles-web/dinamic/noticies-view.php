...
// 	}
	...
	if ($data['IMATGE1'] != '') {
		...
		$data['IMATGE1'] = '<div class="image right clearfix">
						<p class="image"><a href="' . $CONFIG_URLUPLOADIM . '/' . $data['IMATGE1'] . '" title="' . htmlspecialchars($data['PEU_IMATGE1']) . '" class="colorbox"><img src="' . $CONFIG_URLUPLOADIM . '/med-' . $data['IMATGE1'] . '" alt="" /></a></p>
						' . (($data['PEU_IMATGE1'] != '') ? '<p>' . htmlspecialchars($data['PEU_IMATGE1']) . '</p>' : '') . '
					</div>';
		...
		for ($k = 1; $k <= 3; $k++) {
			...
			if ($data["LINK$k"] != '') {
				...
				if ($data["FINESTRA$k"] == 1) {
					...
					$data['RELATED'] .= '<li class="link"><a href="' . $data["LINK1"] . '" rel="external">' . htmlspecialchars($data["TEXTLINK$k"]) . '</a></li>';

				}
				else {

					$data['RELATED'] .= '<li class="link"><a href="' . $data["LINK1"] . '">' . htmlspecialchars($data["TEXTLINK$k"]) . '</a></li>';
