...
// --------------------
   ...
   foreach ($CARDS_FIELDS as $name=>$field)
   {
     ...
     if (isset($_POST[$name]) && $scope=='CUSTOM')
     {
...
//insertar inici descripcioms
 $sql = "insert into PLANTILLA_DESC (PLANTILLA) values ('$id')";
 $result = db_query($sql);
 //fi
// -----------
// REDIRECTION
// -----------
	...
	register_add(t("templateregistrycreate"), "$NOM ($DESCRIPCIO)");

	//fi
  goto_url('descripcions.php?eleccio='.$id);
?>
