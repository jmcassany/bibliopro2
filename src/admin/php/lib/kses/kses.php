...
function kses($string, $allowed_html, $allowed_protocols =
               array('http', 'https', 'ftp', 'news', 'nntp', 'telnet',
                     'gopher', 'mailto'), $auto_closed = null)
###############################################################################
...
###############################################################################
{
  ...
  return kses_split($string, $allowed_html_fixed, $allowed_protocols, $auto_closed);
} # function kses
...
function kses_split($string, $allowed_html, $allowed_protocols, $auto_closed)
###############################################################################
...
###############################################################################
{
  return preg_replace('%(<'.   # EITHER: <
                      ...
                      '$allowed_protocols, $auto_closed)',
                      $string);
} # function kses_split


function kses_split2($string, $allowed_html, $allowed_protocols, $auto_closed)
###############################################################################
...
###############################################################################
{
  ...
  if ($slash != '')
//    return "<$slash$elem>";
    return '<'.$slash.strtolower($elem).'>';  //Afegit xhtml
  # No attributes are allowed for closing elements

//  return kses_attr("$slash$elem", $attrlist, $allowed_html,
//                   $allowed_protocols);
  return kses_attr($slash.strtolower($elem), $attrlist, $allowed_html,
                   $allowed_protocols, $auto_closed);  //Afegit xhtml
} # function kses_split2


function kses_attr($element, $attr, $allowed_html, $allowed_protocols, $auto_closed)
###############################################################################
...
# Is there a closing XHTML slash at the end of the attributes?
  ...
  $xhtml_slash = '';
//  if (preg_match('%\s/\s*$%', $attr) || ($auto_closed != null && ))
  if (preg_match('%\s/\s*$%', $attr) || ($auto_closed != null && in_array($element, $auto_closed)))   //Afegit xhtml
    $xhtml_slash = ' /';
  ...
  while (strlen($attr) != 0)
  {
    ...
    switch ($mode)
    {
      case 0: # attribute name, href for instance
        ...
        {
//          $attrname = $match[1];
          $attrname = strtolower($match[1]); //Afegit xhtml
          $working = $mode = 1;
        ...
        if (preg_match('/^\s+/', $attr)) # valueless
        {
          ...
          $attrarr[] = array
                        ('name'  => $attrname,
                         'value' => '',
//                         'whole' => $attrname,
                         'whole' => "$attrname=\"$attrname\"", //Afegit xhtml
                         'vless' => 'y');
         ...
         # 'value'
        {
          ...
          $attrarr[] = array
                        ('name'  => $attrname,
                         'value' => $thisval,
//                         'whole' => "$attrname=\"$thisval\"",
                         'whole' => "$attrname=\"$thisval\"", //Afegit xhtml
                         'vless' => 'n');
          $working = 1; $mode = 0;
          $attr = preg_replace("/^'[^']*'(\s+|$)/", '', $attr);
          break;
