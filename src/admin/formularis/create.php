...
$result=db_query("select * from FORMULARIS where NOMFORMULARI = '$NOMFORMULARI'");
$row = db_fetch_array($result);
if(!empty($row['ID'])){
  htmlPageError(t("formserrorcreate"));
}
...
$data['MODIFICAT'] = $data['CREATION'];
   ...
   if (!$dbCards->Ok) {
     htmlPageError(t("errordbcardsnew"));
   }
...
// -----------
    ...
    register_add(t("form")." ".t("create"), "$NOMFORMULARI ($TITOLFORMULARI)");
   //fi
