...
$ID=$_GET['ID'];
$FORMULARI=$_GET['FORMULARI'];
$result=db_query("select * from FORMULARIS where ID=$FORMULARI");
...
echo changePage($content, $ID, $FORMULARI);





function changePage($html, $id, $formulari) {
...
<style type="text/css">
  ...
  if (accessGroupPerm('form_edit')) {
    $editEntry = '
        ...
        <a href="'.$CONFIG_URLABSADMIN.'/formularis/items/edita.php?ID='.$id.'" class="menupreview">
...
<table cellpadding="0" cellspacing="0" border="0" style="background-color:#D6D6D6">
	<tr>
		<td style="padding-left:10px;padding-right:40px;width:90px;height:27px">
        <a href="'.$CONFIG_URLABSADMIN.'/formularis/items/index.php?ID='.$formulari.'" class="menupreview">
        <img src="'.$CONFIG_URLABSADMIN.'/comu/bot_generar.gif" alt="'.t("continue").'" width="28" height="19" border="0" align="absmiddle" />
        '.t("continue").'
        </a>
        </td>
'.$editEntry.'
