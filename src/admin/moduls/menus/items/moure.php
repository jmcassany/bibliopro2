...
if ($accio=="down"){
	 ...
	 if($result){
		 ...
		 $result2=db_query_range("select * from MENUITEMS where (ORDRE > '$ORDREACTUALPUJA') AND MENU='$row[MENU]' ORDER BY ORDRE ASC",0,1);
		 ...
		 if ($totalresultats>0){
				...
				$sql = "UPDATE MENUITEMS SET ORDRE='$ORDREACTUALBAIXA' where ID = '$IDPUJA'";
				$result3 = db_query($sql);
				if($result3){
					//ACTUALITZO EL  CAMP Q HA DE BAIXA
					$sql = "UPDATE MENUITEMS SET ORDRE='$ORDREACTUALPUJA' where ID = '$IDBAIXA'";
...
if ($accio=="up"){
	 ...
	 if($result){
		 //selecciono el camp seguent de la base de dades q ha de baixar
		 $result2=db_query_range("select * from MENUITEMS where (ORDRE <'$ORDREACTUALPUJA') AND MENU='$row[MENU]' ORDER BY ORDRE DESC",0,1);
		 $totalresultats=db_num_rows($result2);
		 $row2 = db_fetch_array($result2);
		 $ORDREACTUALBAIXA=$row2['ORDRE'];
		 $IDBAIXA=$row2['ID'];
