...
$NOM=$_POST['NOM'];
$result=db_query("select * from MENUS where NOM = '$NOM'");
$trobats = db_num_rows($result);

if($trobats > 0){
  htmlPageError(t("menuerrorcreate"));
}
...
// -----------
  ...
  if ($CONFIG_MENUACCES) {
    addPerm($id.'_menu');
	  ...
	  register_add(t("menuregistrycreate"), "$NOM($DESCRIPCIO)");
   //fi
