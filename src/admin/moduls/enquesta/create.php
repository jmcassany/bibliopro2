...
//si existeix un menu amb elUSUARIMODI mateix nom no el creem error
$result=db_query("select * from ENQUESTA where NOM = '".$_POST['NOM']."'");
$trobats = db_num_rows($result);

if($trobats > 0){
  htmlPageError(t('pollerror1'));
}
else if (empty($_POST['NOM'])) {
  htmlPageError(t('pollerror2'));
}

$USUARICREAR = $_POST['USUARI'];
$USUARIMODI = $_POST['USUARI'];
...
// -----------
    // Return URL
	  ...
	  register_add("Enquesta creada", "$NOM($DESCRIPCIO)");
   //fi

   goto_url('crearestatic.php?ID='.$id);
?>
