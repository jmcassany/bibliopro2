...
function tractament() {
	...
	$result5 = $db->sql_query("SELECT * FROM news_SUBSCRIPTORS WHERE IdLli = '$ID' AND IdSub='$IDSUB'".$wh_noadmin);
	if ($db->sql_numrows($result5) == 0) {
		htmlPageError('Subscriptor no accessible!');
	}
...
global $CONFIG_PATHUPLOADAD, $UPLOAD_filesize, $UPLOAD_filetype, $CONFIG_URLUPLOADAD;
	...
	if (!eregi($CFG_CAMPANYES['EMAIL_VALID'],$camps['email'])) return 1;
	//list($username,$domain) = split("@",$camps['email']);
	//if (!getmxrr($domain,$mxrecords)) return 1;

	$result1 = $db->sql_query("SELECT * FROM news_SUBSCRIPTORS WHERE IdLli='$ID' AND IdSub!='$IDSUB' AND email='".$camps['email']."' LIMIT 0,1");
	...
	for ($i = 1; $i <=3; $i++) {
		if($_FILES['file'.$i]['name'] != '') {
			...
			if($log2 == 4) {
				...
				$old_query = db_query("SELECT adjunt$i FROM news_SUBSCRIPTORS WHERE IdSub='$IDSUB'");
				...
				db_query("UPDATE news_SUBSCRIPTORS SET adjunt$i='$nom_fitxer_final' WHERE IdSub='$IDSUB'");
			}
		}
	}

	fer_update('news_SUBSCRIPTORS', $camps, "IdLli = '$ID' AND IdSub='$IDSUB' AND IdUsu='$LOGIN'");
