...
function tractament() {
  ...
  $result5 = $db->sql_query("SELECT IdLli,titol FROM news_LLISTES WHERE IdLli != '$ID' $wh_noadmin ORDER BY titol ASC");
...
global $db, $CFG_CAMPANYES, $ID, $LOGIN, $compta;
	...
	$wh_noadmin = " AND IdUsu='$LOGIN'";
	$result5 = $db->sql_query("SELECT IdLli FROM news_LLISTES WHERE IdLli = '$LLISTA'".$wh_noadmin);
	...
	$wh_noadmin = " AND IdUsu='$LOGIN'";
	$result5 = $db->sql_query("SELECT * FROM news_SUBSCRIPTORS WHERE IdLli = '$LLISTA' AND estat='1'".$wh_noadmin);
  while ($row5 = $db->sql_fetchrow($result5)) {
