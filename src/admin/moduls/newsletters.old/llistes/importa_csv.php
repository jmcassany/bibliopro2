...
global $db, $CFG_CAMPANYES, $ID, $LOGIN, $compta;
	...
	//fer_delete('news_SUBSCRIPTORS', "IdLli='$ID'");
    ...
    while(!feof($fp)) {
        $aju = trim(fgets($fp));
        if(strlen($aju)>0) {
        	$aju = ereg_replace('""', 'ARAETCANVIAREPERCOMETA', $aju);
        	$aju = ereg_replace('"', '', $aju);
        	$aju = ereg_replace('ARAETCANVIAREPERCOMETA', '"', $aju);
        	$valors = explode(";", $aju);
        	
        	$insertar['email'] = trim($valors[0]);
        	if (eregi($CFG_CAMPANYES['EMAIL_VALID'],$insertar['email'])) {
			...
			$result1 = $db->sql_query("SELECT email FROM news_SUBSCRIPTORS WHERE IdLli='$ID' AND email='".$insertar['email']."' LIMIT 0,1");
			if ($db->sql_numrows($result1) > 0) {
				//$row1 = $db->sql_fetchrow($result1);
				$compta['nok_duplicat']++;
			} else {
				if (fer_insert('news_SUBSCRIPTORS', $insertar, 0)) $compta['ok']++;
