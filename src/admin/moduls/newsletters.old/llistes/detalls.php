...
function tractament() {
  ...
  $bl['PATH_IMG'] = $CFG_CAMPANYES['PATH_IMG'];
	...
	// LLISTAT DELS SUBSCRIPTORS
		...
		//**** Filtres, Recerques
		$wh_filtre = " WHERE IdLli='$ID' AND IdUsu = '$LOGIN'";
		...
		//**** Paginacions
    $row5 = $db->sql_fetchrow( $db->sql_query("SELECT COUNT(*) AS nombre FROM news_SUBSCRIPTORS ".$wh_filtre) );
  ...
  $result5 = $db->sql_query("SELECT * FROM news_SUBSCRIPTORS ".$wh_filtre.$wh_ordre.$wh_limit);
	...
	$camps['estat'] = intval($_POST['accio']);
 	...
 	if ($camps['estat']==99) {  //eliminar definitivament
	  foreach($_POST['CHECK'] as $k => $v) {
  		if (fer_delete('news_SUBSCRIPTORS', "IdLli = '$ID' AND IdSub='$k' AND IdUsu='$LOGIN'", 0)) $compta['ok']++;
  		else $compta['nok']++;
	  }
	  return -2;
	  ...
	  foreach($_POST['CHECK'] as $k => $v) {
			if (fer_update('news_SUBSCRIPTORS', $camps, "IdLli = '$ID' AND IdSub='$k' AND IdUsu='$LOGIN'", 0)) $compta['ok']++;
			else $compta['nok']++;
	  }
	  return -1;
