...
// ============================================================================
   ...
   accessCheckLevel(2, $CONFIG_PRE_NOMCARPETA.'/admin/');
	...
	$CARDS_LISTFILTER .= " and (TITOL LIKE '%$recerca%' OR RESUM LIKE '%$recerca%' OR  DESCRIPCIO LIKE '%$recerca%' OR NOM LIKE '%$recerca%' OR CARREC LIKE '%$recerca%' OR METATAGS LIKE '%$recerca%')";
...
// ------------------
   ...
   {
      global $CATEGORY1, $CATEGORY2, $MODE, $SKIN, $PAGE, $recerca, $ordre, $ordenar;
      return "busca.php?CATEGORY1=$CATEGORY1&CATEGORY2=$CATEGORY2&MODE=$MODE&SKIN=$SKIN&PAGE=$page&recerca=$recerca&ordre=$ordre&ordenar=$ordenar";
   }
   ...
   $data['PAGELIST']=$pagelist.' ';
	...
	if ($ordenar == "model") {
		$CARDS_LISTSORTBY = 'MODEL '.$ordre;
		$data['COLOR4']="#DEDEDE";
		$data['ICO4']="<img src=\"../../../../../public/media/comu/admin/$ordre.gif\" width=\"10\" height=\"10\"  border=\"0\" align=\"absmiddle\" hspace=\"5\">";
		if ($ordre == "DESC") {
			$ordre = "ASC";
		} else {
			$ordre = "DESC";
   ...
   $total = count($cards);
   if ($total == 0) {
		echo ("<tr><td width=\"20%\">&nbsp;</td><td class=\"text\" colspan=\"3\"><br><b>".$messages["noresul"]."</b></td></tr>");
   }
   ...
   for ($i=0; $i<$total; $i++)
   {
       ...
       foreach ($cards[$i] as $name=>$value)
       {
		...
		if(empty($data['LLOC'])){
		        $data['GESTIO_NOTICIES'] = '
				&#149;&nbsp; <a href="view.php?ID='.$data['ID'].'"><img src="../../../../../public/media/comu/admin/icon_modifica.gif" alt="Editar" width="23" height="16" border="0" align="absmiddle"></a>&#149;&nbsp;
				<img src="../../../../../public/media/comu/admin/icon_borrar.gif" alt="Eliminar" width="22" height="16" border="0" align="absmiddle"><input type="checkbox" name="CHECK_'.$data['ID'].'" value="TRUE" align="absmiddle">
				';
		}else{
			$data['GESTIO_NOTICIES'] = '&#149;&nbsp;<img src="../../../../../public/media/comu/admin/icon_borrar.gif" alt="Eliminar" width="22" height="16" border="0" align="absmiddle"><input type="checkbox" name="CHECK_'.$data['ID'].'" value="TRUE" align="absmiddle">';
		}
