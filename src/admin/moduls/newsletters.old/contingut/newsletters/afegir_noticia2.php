	...
	if ($_REQUEST['add']){
		...
		echo "<script>
				function envia_tanca(){
					...
					parent.opener.location='edita.php?ID='+ID+'&noticia_newsletter='+noticia_newsletter+'&id_cat=0&IdCam='+IdCam;
					self.close(); 
				}
			</script>";
		...
		$opcions = "<form action=\"afegir_noticia2.php\" method=\"post\" name=\"form_sel\"><BR>
...
<!-- CERCADOR -->
<form action="afegir_noticia2.php">	
...
<?php
	if ($_REQUEST['accion']){
		...
		echo "<form action=\"afegir_noticia2.php\" method=\"post\">";
