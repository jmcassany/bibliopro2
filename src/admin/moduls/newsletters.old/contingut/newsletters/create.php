...
// ============================================================================
	...
	//miro IDNL de l'�ltim newsletter ja existent...
   	$result = mysql_query("select MAX(IDNL) from NEWSLETTERS");
...
// -----------
	...
	$result = mysql_query("select MAX(ID) from NEWSLETTERS");
	$row = mysql_fetch_array($result);
	$id = $row['0'];
	
	header("Location: edita.php?ID=".$id."&IdCam=".$_POST['IdCam']);
