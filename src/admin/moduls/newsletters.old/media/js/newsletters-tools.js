	function launchPreview(address) {
		void(window.open(address,'preview','height=500,width=700,resizable=yes,scrollbars=yes'));
		return false;
	}
	...
	function checkDeleteList(campaignID,ListID,Count) {	
		...
		if (confirm(msg))
			location.replace("resum.php?id=" + campaignID + "&llista=" + ListID + "&accio=eliminadesti");			
		return false;
	}
