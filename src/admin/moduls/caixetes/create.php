...
$result=db_query("select * from CAIXETES where NOM = '$NOM'");
$trobats = db_num_rows($result);

if($trobats > 0){
  htmlPageBasicError(t('bannerserror1'));
}
...
/*pujar imatge alternativa per flash*/
  $log = 1;
  if($_FILES['img1']['name'] != '') {
...
// -----------
    ...
    // Return URL
	  ...
	  register_add("Caixeta creada", "$NOM($DESCRIPCIO)");
   //fi
