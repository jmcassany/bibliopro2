...
//si existeix un menu amb el mateix nom no el creem error
$result=db_query("select * from VIEWRSS where NOM = '".$_POST['NOM']."' AND ID != '".$_POST['ID']."'");
$trobats = db_num_rows($result);

if($trobats > 0){
  htmlPageError(t('bannersgrouperror1'));
}
if (empty($_POST['NOM'])) {
...
// -------------
   ...
   // Return URL
	 ...
	 register_add("Visualitzador rss actualitzat", "$NOM ($DESCRIPCIO)");
   //fi
