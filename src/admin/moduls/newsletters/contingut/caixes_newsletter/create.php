...
$STATUS = $_POST['STATUS'];
$sql = "INSERT INTO " . TAULA_CAIXETES . " (CLASS,SKIN,CATEGORY1,CATEGORY2,STATUS,VISIBILITY,CREATION,START,END,TITOL,USUARI_HOUDINI,TEXT) values ('1','0','$CATEGORY1','$CATEGORY2','$STATUS','1','$avui','$start','$end','$TITOL','$USUARI_HOUDINI','$TEXT')";
$result = db_query($sql);

if($result) {

    $result = db_query("SELECT ID FROM " . TAULA_CAIXETES . " ORDER BY ID DESC LIMIT 1");
    ...
    if($result) {
        ...
        if(isset($_FILES['img']) && $_FILES['img']['name'][0] != ''){
            ...
            $abpath = $CONFIG_PATHUPLOADCAIXES; //ruta absoluta on es puja la imatge
                ...
                if (($sizelim == "yes") && ($img['size'][0] > $size)) {
                    ...
                    if (in_array($img['type'][0],$allowed)) {
                        ...
                        if (file_exists($abpath . $posanom)) {
                            ...
                            $sql = "UPDATE " . TAULA_CAIXETES . "  SET IMATGE='$posanom'  where ID='$ID'";
                            $result = db_query($sql);
