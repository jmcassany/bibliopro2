<?php
	...
	if (!empty($cerca)) {
		...
		$CARDS_LISTFILTER = "PAIS LIKE '%$cerca%'";
	}
	...
	if ($ordenar == "ral"){
		$CARDS_LISTSORTBY = "REDUCCION_AL $ordre, PAIS ASC";
		$data['COLOR2']="#DEDEDE";
  ...
  $data['CONFIG_URLADMIN'] = $CONFIG_URLADMIN;
	...
	for ($i=0; $i<$total; $i++)
	{
		...
		if ($data['REDUCCION_AL'] == 1) { $data['REDUCCION_AL'] = 'Sí'; }
		else { $data['REDUCCION_AL'] = 'No'; }

		if ($data['CE'] == 1) { $data['CE'] = 'Sí'; }
		else { $data['CE'] = 'No'; }
