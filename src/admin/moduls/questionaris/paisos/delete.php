<?php
	...
	if (!isset($_POST['CONFIRM']) || $_POST['CONFIRM'] !='TRUE') {
	  htmlPageError('Error esborrant pais de qüestionaris');
	}
	...
	if (!isset($_POST['CHECK'])) {
	  $missatge = t("errordeletenocheck");
		...
		foreach($_POST['CHECK'] as $key => $value) {
			...
			/*obtenir les dades*/
			$result = db_query('select PAIS, ID from ' . $CARDS_TABLE . ' where ID = '.$key);
...
// 			) == 0) {
				...
				register_add("Pais de qüestionaris eliminat", $row['PAIS']);
...
?>
	<tr>
		<td class="text">El pais <em><?php echo $notDeletedCountry; ?></em> <strong>no</strong> s'ha pogut esborrar ja que està seleccionat en algun dels qüestionaris.</td>
	</tr>
<?php
...
?>
	<tr>
		<td class="text">Els països de qüestionaris seleccionats han estat esborrats</td>
	</tr>
	<tr>
