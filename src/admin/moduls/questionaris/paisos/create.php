<?php
	...
	// si existeix un pais amb el mateix nom no el creem
	$result=db_query("select * from $CARDS_TABLE where PAIS = '$PAIS'");
	$trobats = db_num_rows($result);

	if($trobats > 0){
		htmlPageBasicError('Ja existeix un pais amb el nom indicat.');
	}
	...
	register_add("Pais de qüestionaris creat", "$PAIS");
	//fi
