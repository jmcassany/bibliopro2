<?php
	...
	if (!empty($cerca)) {
		...
		$CARDS_LISTFILTER .= " AND (
			...
			ENTIDAD_NOMBRE LIKE '%$cerca%' OR
			ENTIDAD_DIRECCION LIKE '%$cerca%' OR
			ENTIDAD_CIUDAD LIKE '%$cerca%' OR
			FACTURACION_NOMBRE LIKE '%$cerca%' OR
			FACTURACION_DIRECCION LIKE '%$cerca%' OR
			FACTURACION_CIUDAD LIKE '%$cerca%' OR
			FACTURACION_CIF LIKE '%$cerca%' OR
			FACTURACION_EMAIL LIKE '%$cerca%'
		)";
	}
	// pais
	if (!empty($pais)) {
		$pais = addslashes($pais);
		$CARDS_LISTFILTER .= " AND PAIS = '$pais'";
	}
	// tipus entitat
	...
	if (!empty($tipus_centre)) {
		$CARDS_LISTFILTER .= " AND TIPO_CENTRO_TRABAJO = '$tipus_centre'";
	}
	...
	if ($ordenar == "nom"){
		$CARDS_LISTSORTBY = "NOMBRE $ordre, EMAIL ASC";
		$data['COLOR2']="#DEDEDE";
		$data['ICO2']="<img src=\"../../../comu/$ordre.gif\" width=\"10\" height=\"10\" alt=\"Ascendent\" border=\"0\" align=\"absmiddle\" hspace=\"5\">";
		if ($ordre == "DESC") { $ordre = "ASC"; }
		else { $ordre = "DESC"; }
	}
	if ($ordenar == "newsletter"){
		$CARDS_LISTSORTBY = "NEWSLETTER $ordre, EMAIL ASC, NOMBRE ASC";
		$data['COLOR3']="#DEDEDE";
  ...
  $data['CONFIG_URLADMIN'] = $CONFIG_URLADMIN;
	...
	// generem options països a partir de la base de dades
	$countriesQuery = db_query("
		SELECT ID, PAIS FROM `$COUNTRIES_TABLE`
		ORDER BY PAIS ASC
	");
	$data['OPTIONS_PAISOS'] = '<option value="">Quaselvol</option>';
	while ($countriesRow = db_fetch_array($countriesQuery)) {
		$data['OPTIONS_PAISOS'] .= '<option value="' . $countriesRow['ID'] . '">' . htmlspecialchars($countriesRow['PAIS']) . '</option>';
		// si es cerca per país, obtenim string
		if (!empty($pais)) {
			if ($countriesRow['ID'] == $pais) { $filteredCountry = $countriesRow['PAIS']; }
		}
	}
	$data['OPTIONS_PAISOS'] .= '<option value="0">Otro</option>';

	$data['OPTIONS_TIPO_ENTIDAD'] = generateOptions($entityTypes);

	$data['OPTIONS_TIPO_CENTRO_TRABAJO'] = generateOptions($workCenterTypes);
	...
	if (
		...
		!empty($tipus_centre) or
		$subs != ''
	) {
		$c = '<ul>';
		if (!empty($cerca)) {
			$c .= "<li>Text: <strong>$cerca</strong></li>";
		}
		if (!empty($pais)) {
			$c .= "<li>Pais: <strong>" . htmlspecialchars($filteredCountry) . "</strong></li>";
		}
		if (!empty($tipus_entitat)) {
			$c .= "<li>Tipus entitat: <strong>" . $entityTypes[$tipus_entitat] . "</strong></li>";
		}
		if (!empty($tipus_centre)) {
			$c .= "<li>Tipus centre treball: <strong>" . $workCenterTypes[$tipus_centre] . "</strong></li>";
		}
		if ($subs != '') {
			$c .= "<li>Subscripció: <strong>$subs</strong></li>";
		}
		$c .= '</ul>';
		if ($total == 0) {
			$data['RESULTAT'] = "<tr>
	...
	for ($i=0; $i<$total; $i++)
	{
		...
		if ($data['NEWSLETTER'] == 1) { $data['NEWSLETTER'] = 'Sí'; }
		else { $data['NEWSLETTER'] = 'No'; }

		// OUTPUT ROW =====================================================
		// si es filtra per subscripció, decidim si mostrem o no l'usuari
		if ($subs != '') {

			$subs = addslashes($subs);
			$scq = db_query("
				SELECT ID FROM `$SUBSCRIPTIONS_TABLE`
				WHERE
					USUARIO = $data[ID] AND
					FECHA_ACTIVACION > '" . date('Y-m-d H:i:s', time() - $secondsPerYear) . "'
			");

			if ($subs == 1 and db_num_rows($scq) > 0) {

				echo $Tpl->mergeBlock('ROW',$data);

			}
			elseif ($subs == 0 and db_num_rows($scq) == 0) {
