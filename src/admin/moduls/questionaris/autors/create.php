<?php
	...
	// si existeix un autor amb el mateix nom no el creem
	$result=db_query("select * from $CARDS_TABLE where NOM = '$NOM'");
	$trobats = db_num_rows($result);

	if($trobats > 0){
		htmlPageBasicError('Ja existeix un autor amb el nom indicat.');
	}
	else if (empty($_POST['NOM'])) {
		htmlPageBasicError('No s\'ha indicat cap nom per a l\'autor');
	}
	...
	register_add("Autor de qüestionaris creat", "$NOM");
	//fi
