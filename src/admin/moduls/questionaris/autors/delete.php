<?php
	...
	if (!isset($_POST['CONFIRM']) || $_POST['CONFIRM'] !='TRUE') {
	  htmlPageError('Error esborrant autor de qüestionaris');
	}
	...
	if (!isset($_POST['CHECK'])) {
	  $missatge = t("errordeletenocheck");
		...
		foreach($_POST['CHECK'] as $key => $value) {
			...
			/*obtenir les dades*/
			$result = db_query('select NOM, ID from ' . $CARDS_TABLE . ' where ID = '.$key);
			...
			if (db_num_rows(
				...
				register_add("Autor de qüestionaris eliminat", $row['NOM']);
...
<?php
	...
	if (count($notDeleted) > 0) {
		...
		foreach ($notDeleted as $notDeletedAuthor) {

?>
	<tr>
		<td class="text">L'autor <em><?php echo $notDeletedAuthor; ?></em> <strong>no</strong> s'ha pogut esborrar ja que està seleccionat en algun dels qüestionaris.</td>
	</tr>
<?php
...
?>
	<tr>
		<td class="text">Els autors de qüestionaris seleccionats han estat esborrats</td>
	</tr>
	<tr>
