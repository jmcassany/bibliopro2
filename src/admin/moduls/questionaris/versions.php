<?php
	...
	// filtre id cuestionari
	if (!empty($_GET['ID_CUEST'])) {
		$CARDS_LISTFILTER = "ID_CUEST = '$_GET[ID_CUEST]'";
	}
	...
	if (!empty($cerca)) {
		...
		$filter = "(
				NOM_ORIGINAL LIKE '%$cerca%' OR
				NOM_CAST LIKE '%$cerca%' OR
				COPYRIGHT_ORIGINAL LIKE '%$cerca%' OR
				COPYRIGHT_CAST LIKE '%$cerca%' OR
				EMAIL_CONTACTO_ORIGINAL LIKE '%$cerca%' OR
				EMAIL_CONTACTO_CAST LIKE '%$cerca%' OR
				PALABRAS_CLAVE LIKE '%$cerca%'
		)";
		if (!empty($_GET['ID_CUEST'])) {
			$CARDS_LISTFILTER .= "AND ($filter)";
		}
		else {
			$CARDS_LISTFILTER = $filter;
		}
	}
	...
	$Tpl = new awTemplate();
	$Tpl->scanFile("versions.tpl");

	// Si hi ha cap problema -> Error
	if (!$Tpl->Ok) { echo "<B>Error: No se ha encontrado la plantilla 'versions.tpl'.</B><br>\n"; exit; }
	...
	if ($ordenar == "version"){
		$CARDS_LISTSORTBY = 'VERSION' . $ordre;
		$data['COLOR2']="#DEDEDE";
  ...
  $data['CONFIG_URLADMIN'] = $CONFIG_URLADMIN;
	...
	for ($i=0; $i<$total; $i++)
	{
		...
		// generem cadena de text amb els autors
		$autors = unserialize($data['IDAUTORES_ORIGINAL']);
		$nomAutors = array();
		foreach ($autors as $autorID) {

			$aQuery = db_query("SELECT NOM FROM $AUTHORS_TABLE WHERE ID=$autorID");
			$aRow = db_fetch_array($aQuery);
			$nomAutors[] = $aRow['NOM'];


		}
		$data['AUTORS'] = implode(', ', $nomAutors);

		// si és qüestionari pare, permetem crear-ne nova versió
		if ($data['VERSION'] == 1) {
			$data['LINK_DUPLICAR'] = '<a href="duplicate.php?ID=' . $data['ID'] . '"><img src="' . $CONFIG_URLADMIN . '/comu/icon_duplica.gif" alt="Duplicar" width="20" height="16" border="0" align="absmiddle" ></a>';
		}
		else {
			$data['LINK_DUPLICAR'] = '';
		}
