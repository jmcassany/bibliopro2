    ...
    $.extend(settings, options);
		...
		return this.each(function() {
			...
			if (collapsingContainer.children('.'+settings.classes.collapse).length == 0) {
				collapsingContainer.prepend('<span class="'+settings.classes.collapse+'"><a href="#">'+settings.i18n.collapse+'</a></span>');
			}
	...
	$.fn.makeRemovable = function(options) {
		...
		return this.each(function() {
			...
			if (controls.length > 0) {
				...
				controlsContainer.prepend('<ul class="'+settings.classes.controls+'"><li class="'+settings.classes.remove+'"><a href="#">'+settings.i18n.remove+'</a></li></ul>');
			}
		...
		return this.each(function() {
			...
			box.find(settings.elements.title).click(function() {
				...
				if (input.length > 0) {
					...
					title.hide();
					var input = $(this).parent().children('#title'+randomNumber);
				}
