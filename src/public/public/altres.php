    ...
    if($dadesButlleti && $dadesCampanya){
        ...
        switch($opcio){
            ...
            case 'legal':
                ...
                $contingut = sprintf($contingut, $default['email'],$default['email']);
                break;
            case 'alta':
            ...
            case 'anteriors':
                ...
                $consultaAnteriors = db_query('SELECT * FROM ' . TAULA_CAMPANYES . ' as c INNER JOIN ' . TAULA_NEWSLETTERS . ' AS nl ON (c.IdCam = nl.IdCam) WHERE estat >= 101 AND nl.SKIN = 0');
            ...
            case 'detall': //Només per notícies del newsletter agafem el model hotitzontal per defecte
                if(isset($_GET['idNot'])){
                    ...
                    //Per poder veure la notícia sense sumar visites des del gestor:
                    if(!isset($_GET['clics'])){
                        //Control de visites
                        $ipVisitant = $_SERVER['REMOTE_HOST'];
                        $sqlClics = db_query("SELECT * FROM " . TAULA_CLICSNOTICIES . " WHERE ID_NL = " . $dadesButlleti['ID'] . " AND ID_NNL = " . $_GET['idNot']);
                        if(db_num_rows($sqlClics) > 0){
                            while($rowClic = db_fetch_array($sqlClics)){
                                if($rowClic['GALETA'] != $ipVisitant){
                                    db_query('INSERT INTO ' . TAULA_CLICSNOTICIES . ' (ID_NNL, ID_NL, GALETA) VALUES("' . $_GET['idNot'] . '","' . $dadesButlleti['ID'] . '","' . $ipVisitant . '")');
                                    db_query('UPDATE ' . TAULA_NLTONOTICIES . ' SET LINKS = LINKS + 1 WHERE ID_NNL = "' . $_GET['idNot'] . '" AND ID_NL = "' . $dadesButlleti['ID'] . '"');
                                }
                            }
                        } else {
                            db_query('INSERT INTO ' . TAULA_CLICSNOTICIES . ' (ID_NNL, ID_NL, GALETA) VALUES("' . $_GET['idNot'] . '","' . $dadesButlleti['ID'] . '","' . $ipVisitant . '")');
            ...
            case 'banner':
                if(isset($_GET['idBan'])){
                    $queryBanner = db_query('SELECT * FROM ' . TAULA_BANNERS . ' WHERE STATUS=1 AND ID = "' . $_GET['idBan'] . '" LIMIT 0,1');
                    if($queryBanner && db_num_rows($queryBanner) > 0){
                        $dadesBanner = db_fetch_array($queryBanner);
                        //Control de visites
                        $ipVisitant = $_SERVER['REMOTE_HOST'];
                        $sqlClics = db_query("SELECT * FROM " . TAULA_CLICSBANNERS . " WHERE ID_NL = " . $dadesButlleti['ID'] . " AND ID_BAN = " . $dadesBanner['ID']);
                        if(db_num_rows($sqlClics) > 0){
                            while($rowClic = db_fetch_array($sqlClics)){
                                if($rowClic['GALETA'] != $ipVisitant){
                                    db_query('INSERT INTO ' . TAULA_CLICSBANNERS . ' (ID_BAN, ID_NL, GALETA) VALUES("' . $dadesBanner['ID'] . '","' . $dadesButlleti['ID'] . '","' . $ipVisitant . '")');
                                    db_query('UPDATE ' . TAULA_NLTOBANNERS . ' SET LINKS = LINKS + 1 WHERE ID_NL = "' . $dadesButlleti['ID'] . '" AND ID_BAN = "' . $dadesBanner['ID'] . '"');
                                }
                            }
                        } else {
                            db_query('INSERT INTO ' . TAULA_CLICSBANNERS . ' (ID_BAN, ID_NL, GALETA) VALUES("' . $dadesBanner['ID'] . '","' . $dadesButlleti['ID'] . '","' . $ipVisitant . '")');
                        ...
                        header('Location:' . $dadesBanner['LINK']);
                        exit;
                    }
                            ...
                            if ($row5['estat'] == 0) {  //si encara no s'ha enviat és un test i no cal actualitzar
                                ...
                                if (fer_update(TAULA_DESTINATARIS, $camps, "IdCam = '$idcam' AND email='$email'", 0)) {
                                    if ($row5['IdLli'] == 0) { //afegit manualment!
                                        ...
                                        $result9 = db_query("SELECT estat FROM " . TAULA_SUBSCRIPTORS . " WHERE email='$email'");
        ...
        echo $contingut = '<h4>' . $messages['errorparam'] . ' 2</h4>';
    }
} else {
    echo $contingut = '<h4>' . $messages['errorparam'] . ' 1</h4>';
}
